...
endpoints = do
    get "/api/orders" $ do
        ...
        json (map toDTO result :: [OrderDto])

    post "/api/orders" $ do
        (payload :: OrderPayloadDto) <- parseBody
        ...
        case result of
            ...
            Left (OnAddressResolution NoCoordinatesFound) -> do
                status badRequest400
                json ("Unable to identify order address" :: T.Text)
            Left (OnAddressResolution (AmbiguousCoordinates addresses)) -> do
                status badRequest400
                json $ object
                    [ "error" .= ("Provided address is ambiguous" :: T.Text)
                    , "variants" .= (unAddress <$> addresses :: [T.Text])
                    ]
